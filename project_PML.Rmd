...
```
in the figure there is no pattern, so try 10 pcaComp
```{r}
PCAf<- preProcess(tr[,-length(tr)], method="pca", pcaComp = 10)
trPCA<-predict(PCAf,tr[,-length(tr)])
```
...
Random Ferest
```{r}
modFitRF<-train(classe~.,method='rf', data=tr, prox=TRUE)
```